# Implementation Prompt for RFC-002: Organizer Runbook (Supabase Studio Workflows)
...
## Context
This implementation covers RFC-002: the organizer-facing Supabase Studio workflows — create/activate a tournament, the deliberate two-step activation switch, player CRUD without redeploy, and the fast 20-player setup — captured as `knowledge/RUNBOOK.md` and validated against the RFC-001 database (also seeding the test data later RFCs rely on). Refer to:
...
- RFC-002.md for the specific requirements being implemented
...
## Scope Limitation
Only implement features in RFC-002.md. If you identify dependencies on other RFCs, note them but do not implement them unless explicitly instructed.
