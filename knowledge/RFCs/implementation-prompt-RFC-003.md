# Implementation Prompt for RFC-003: Configuration & PostgREST Data-Access Seam
...
## Context
This implementation covers RFC-003: Supabase configuration (env vars + fail-fast startup validation, `.env.example`) and the single thin async data-access helper that reads the active tournament's matching player via raw PostgREST over the existing `aiohttp`, with a bounded timeout and a never-raising sentinel contract. Refer to:
...
## Scope Limitation
Only implement features in RFC-003.md. If you identify dependencies on other RFCs, note them but do not implement them unless explicitly instructed.
