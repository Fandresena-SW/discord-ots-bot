# Implementation Prompt for RFC-001: Supabase Schema, Constraints, Indexes & Trigger
...
## Context
This implementation covers RFC-001: the Supabase data layer — the `tournaments` and `players` tables, the single-active-tournament and case-insensitive/whitespace-trimmed unique-name constraints, the read-path indexes, and the name-trimming trigger, delivered as an idempotent `schema.sql`. Refer to:
...
- RFC-001.md for the specific requirements being implemented
...
## Scope Limitation
Only implement features in RFC-001.md. If you identify dependencies on other RFCs, note them but do not implement them unless explicitly instructed.
